...
$(".addVersion").on("click",function(){
	var obj = $(this);
	window.location.href="/dev/appversionadd?id="+obj.attr("appinfoid");
});
$(".modifyVersion").on("click",function(){
	...
	if(status == "1" || status == "3"){//待审核、审核未通过状态下才可以进行修改操作
		if(versionid == null || versionid == ""){
			...
			window.location.href="/dev/appversionmodify?vid="+ versionid + "&aid="+ appinfoid;
		}
	}else{
...
$(document).on("click",".saleSwichOpen,.saleSwichClose",function(){
	...
	if("open" === saleSwitch){
		...
		if(confirm(appinfoid+"你确定要下架您的APP应用【"+obj.attr("appsoftwarename")+"】吗？")){
			saleSwitchAjax(appinfoid,obj);
		}
	}
});
